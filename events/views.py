...
def search_events(request):
    ...
    Performs a fuzzy search on 'title' and 'category' fields.
    """
...
def event_register(request, event_id):
    ...
    if request.method == 'POST':
        ...
        has_conflict = Registration.objects.filter(
        ...
        if has_conflict:
            return render(request, 'event_registration.html', {
                'event': event,
                'error_msg': 'Time Conflict: You are already registered for another event at this time!'
            })
